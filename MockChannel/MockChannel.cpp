...
#include"../MPC/ApplicationHelpers/AppHelpers.h"
#include "../MPC/FileSystem/FileSystem.h"
#include <string>
#include <thread>
#include <iostream>
...
{
public:
  MockChannel(ISendr* pSendr, IRecvr* pRecvr, std::string port);
  void start();
  void stop();
private:
  std::thread thread_;
...
MockChannel::MockChannel(ISendr* pSendr, IRecvr* pRecvr, std::string port) : pISendr_(pSendr), pIRecvr_(pRecvr), cli("localhost", stoi(port)) {}
...
IMockChannel* ObjectFactory::createMockChannel(ISendr* pISendr, IRecvr* pIRecvr, std::string port) 
{ 
  return new MockChannel(pISendr, pIRecvr, port); 
}
